...
module SwaggerEventSchema1
  ...
  module ClassMethods
    ...
    def swagger_event_schema(name, skip_session_fields: false, type:, &block)
      schema = swagger_schema(name) do
        ...
        unless skip_session_fields
          property :session_uuid do
            key :type, :string
            key :format, 'uuid'
            key :description, 'The UUID of the current session, previously generated by the client and submitted in a "session started" event.'
          end
          property :session_order do
            key :type, :integer
            key :description, "The event's numerical order within this session. E.g. the first event after a session " \
                            "is started should give 0 for this field, the next one should give 1, etc."
         end
        end
      end
