# Forked from https://github.com/dasch/avro_turf/blob/dae9eba3246d67a903bc775600e1d544bdd7c1e7/lib/avro_turf/schema_store.rb
# Would be nicer to make a PR to avro_turf that makes the mapping of path and fullname to an AVSC file configurable

class EventCaptureSchemaStore
  ...
  def load_schema!(fullname, namespace = nil, local_schemas_cache = {})
    schema_path = build_schema_path(@path, fullname)
  ...
  def build_schema_path(path, fullname)
    *namespace, schema_name = fullname.split(".")
    schema_name_without_version, version = schema_name.split(/_(v\d+)\z/)
    schema_path = File.join(path, *namespace, schema_name_without_version, version, "avro.avsc")
  end
end
